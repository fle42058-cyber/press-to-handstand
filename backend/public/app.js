...
function makeHttpBackend() {
    ...
    return {
        ...
        recommend: async (caps) => await (await fetch("/api/recommend", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ caps }) })).json(),
        warm: () => { },
    };
}
let backend;
...
function el(tag, cls) { const e = document.createElement(tag); if (cls)
...
function drawLine(cv, data, opts) {
    ...
    const X = (i) => padL + (i / (n - 1)) * iw, Y = (v) => padT + ih * (1 - (v - mn) / (mx - mn));
    const grad = ctx.createLinearGradient(0, padT, 0, padT + ih);
    grad.addColorStop(0, "rgba(79,209,197,.30)");
...
async function pollEvolution() {
    try {
        ...
        drawLine($("evoChart"), d.history.length ? d.history : [d.best_fitness], { color: "#4fd1c5" });
    }
    catch (_) { }
}
// ---------- sliders ----------
...
async function recommend() {
    ...
    const d = await backend.recommend(curCaps);
    renderPlan(d);
}
function renderPlan(d) {
    ...
    const bar = $("pBar");
    bar.style.width = Math.min(100, d.finalAttainment * 100) + "%";
    bar.style.background = reached ? "linear-gradient(90deg,#3ddc97,#4fd1c5)" : "linear-gradient(90deg,#ffb454,#4fd1c5)";
    const src = $("planSrc");
    src.className = "pill " + (d.source === "evolved" ? "ok" : "start");
    src.textContent = d.source === "evolved" ? "evolved NN" : "greedy";
    planChart = d.trace;
    drawLine($("planChart"), d.trace, { min: 0, max: 1, color: reached ? "#3ddc97" : "#ffb454" });
    const bl = $("pBlocks");
    bl.innerHTML = "";
    d.blocks.forEach((b, i) => {
        const row = el("div", "block");
        const phase = el("div", "phase");
        phase.textContent = "WK " + (b.weekStart + 1) + "–" + (b.weekStart + b.weeks);
        const desc = el("div", "desc");
        desc.textContent = (i + 1) + ". " + b.name;
        const focus = el("span", "focus");
        focus.textContent = b.weeks + " week" + (b.weeks > 1 ? "s" : "") + " primary focus";
        desc.appendChild(focus);
        row.append(phase, desc);
        bl.appendChild(row);
    });
    const cb = $("capBars");
    cb.innerHTML = "";
    ...
    meta.capacities.forEach((c, i) => {
        ...
        const barW = el("div", "bar");
        barW.style.position = "relative";
        const a = el("i");
        a.style.width = (start * 100) + "%";
        a.style.background = "#5b6f8a";
        a.style.position = "absolute";
        a.style.top = "0";
        a.style.left = "0";
        const e = el("i");
        e.style.width = (end * 100) + "%";
        e.style.background = "linear-gradient(90deg,#4fd1c5,#7aa2ff)";
        e.style.height = "100%";
        barW.appendChild(a);
        barW.appendChild(e);
        const lbl = el("div", "lbl");
        lbl.textContent = Math.round(start * 100) + "%→" + Math.round(end * 100) + "%";
        row.append(nm, barW, lbl);
        cb.appendChild(row);
    });
